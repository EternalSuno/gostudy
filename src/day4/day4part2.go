...
func concurrent() {
	...
	//
	//Select
	//我们上面介绍的都是只有一个 channel 的情况，那么如果存在多个 channel 的时候，
	//我们该如何操作呢，Go 里面提供了一个关键字 select，通过 select 可以监听 channel 上的数据流动。
	...
	//	fibonacci(c, quit)
	//}
	//在 select 里面还有 default 语法，select 其实就是类似 switch 的功能，
	//default 就是当监听的 channel 都没有准备好的时候，默认执行的（select 不再阻塞等待 channel）。
	//select {
	//case i := <-c:
	//    // use i
	//default:
	...
	//超时
	//有时候会出现 goroutine 阻塞的情况，那么我们如何避免整个程序进入阻塞的情况呢？我们可以利用 select 来设置超时
	//
	//func main() {
	//	c := make(chan int)
	//	o := make(chan bool)
